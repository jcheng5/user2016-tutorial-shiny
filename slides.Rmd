...
## Abort computations with `req()`

**Use `req()` to halt the current reactive/observer/output if a prerequisite condition is not met.**

- `selectInput` has no item selected
- No text entered into `textInput`
- No value in `numericInput`
- The input in question hasn't been added to the page at all

## Example

> - **`req_v1_broken.R`**  
Doesn't even work, too many errors

> - **`req_v2_badfix.R`**  
Naive approach---manually check every little precondition (verbose, error-prone!)

> - **`req_v3_goodfix.R`**  
Much more concise and robust approach
